...
<div>
    ...
    <div style="width: 100%;">
        <#list data as d>
            <i class="layui-icon layui-icon-rate" onclick="changeCss(this)" value="${d.courseName!''}" style="margin: 10px;padding: 2px;font-size: 24px;font-weight: 700">${d.courseName!""}</i>
        </#list>
    </div>
...
<script>
   ...
   /* $('#new').on('click', function(){
    ...
    $('#transmit').on('click', function(){
        parent.$('#parentCourseIframe').text(chooseVal);
        parent.layer.tips('Look here', '#parentCourseIframe', {time: 5000});
