layui.use(['table','layer',"form"],function(){
    ...
    var  tableIns = table.render({
        elem: '#searchList',
        url : ctx+'/search/auditResearchList',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
        ...
        cols : [[
            ...
            {field: 'restate', title: '课题状态',  width: 120, align: 'center', templet: function (d) {
                    if (d.restate == true) {
    ...
    table.on('tool(searchList)', function (data) {
        ...
        if (data.event == "edit") { // 更新客户信息
            ...
            // 打开添加/修改客户信息的对话框
            openAddOrUpdateResearchDialog(data.data.id);

        } else if (data.event == "del") { // 删除客户信息

            // 删除客户
            deleteResearch(data.data.id);
        }else if(data.event == "searchDetail"){ //查询详细信息
            openResearchDialog(data.data.id);
        }
     ...
     * 打开添加/修改客户信息的对话框
     */
    function openAddOrUpdateResearchDialog(id) {
        var title = "<h3>课题信息管理 - 添加课题信息</h3>";
        var url = ctx + "/search/toAddOrUpdateAuditResearchPage";

        // 判断id是否为空 （如果不为空，则为更新操作）
        if (id != null && id != '') {
            title = "<h3>课题信息管理 - 更新课题信息</h3>";
            url = ctx + "/search/toAddOrUpdateAuditResearchPage?id="+id;
        }
    ...
    function deleteResearch(id) {
        // 弹出确认框，询问用户是否确认删除
        layer.confirm('确定要删除该记录吗？',{icon:3, title:"课题信息管理"}, function (index) {
            // 关闭确认框
            layer.close(index);
     ...
     * @param data
     */
    function openResearchDialog(data) {
        // 判断用户是否选择客户
        if (data.length == 0) {
        ...
        layui.layer.open({
            ...
            area: ['700px', '500px'],
            // url地址
            content: ctx + "/search/toResearchAuditDetail?id=" + data,
            // 可以最大化与最小化
            maxmin:true
        });
