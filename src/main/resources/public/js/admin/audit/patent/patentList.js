layui.use(['table','layer',"form"],function(){
    ...
    var  tableIns = table.render({
        elem: '#patentList',
        url : ctx+'/patent/auditPatentList',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
    ...
    table.on('tool(patentList)', function (data) {
        ...
        if (data.event == "edit") { // 更新客户信息
            ...
            // 删除客户
            deletePatent(data.data.patid);
        } else if(data.event == "patentDetail"){ //查询详细信息
    ...
    function openAddOrUpdatePatentDialog(id) {
        ...
        var url = ctx + "/patent/toAddOrUpdateAuditPatentPage";

        // 判断id是否为空 （如果不为空，则为更新操作）
        if (id != null && id != '') {
            title = "<h3>专利信息管理 - 更新专利信息</h3>";
            url = ctx + "/patent/toAddOrUpdateAuditPatentPage?patid="+id;
        }
    ...
    function deletePatent(id) {
        // 弹出确认框，询问用户是否确认删除
        layer.confirm('确定要删除该记录吗？',{icon:3, title:"专利信息管理"}, function (index) {
            // 关闭确认框
            layer.close(index);
    ...
    function openPatentDialog(data) {
        ...
        layui.layer.open({
            ...
            area: ['700px', '500px'],
            // url地址
            content: ctx + "/patent/toPatentAuditDetail?patid=" + data,
            // 可以最大化与最小化
            maxmin:true
        });
