layui.use(['table','layer',"form"],function(){
    ...
    var  tableIns = table.render({
        elem: '#classHourList',
        url : ctx+'/classHour/auditClassHourList',
        cellMinWidth : 95,
        page : true,
        height : "full-125",
        limits : [10,15,20,25],
        ...
        cols : [[
            ...
            {title:'操作',templet:'#classHourListBar', fixed: 'right', align:'center', minWidth:300}
        ]]
    });
    ...
    table.on('tool(classHourList)', function (data) {
        ...
        if(data.event=="classHourDetail"){
            ...
            openClassHourAuditDialog(data.data.id);
        }

    });

    function openClassHourAuditDialog(data) {
        // 判断用户是否选择客户
        if (data.length == 0) {
        ...
        layui.layer.open({
            ...
            type: 2,
            // 标题
            title: "<h3>课时信息管理 - 课时审核</h3>",
            // 宽高
            area: ['700px', '500px'],
            // url地址
            content: ctx + "/classHour/classHourAudit?id=" + data,
            // 可以最大化与最小化
            maxmin:true
        });
    }
