layui.use(['form', 'layer' ,'upload','jquery','element'], function () {
     ...
     * 表单Submit监听
     */
    form.on('submit(addOrUpdateResearch)', function (data) {

        var restate;
        if($("input[name='restate']").val()=="未完成"){
            restate=false;
        }else{
            restate=true;
        }
        ...
        // 请求的地址
        var url = ctx + "/search/addResearch?restate="+restate; // 添加操作
        ...
        if ($("#id").val()) {
            var id=$("#id").val();
            // 更新操作
            var url = ctx + "/search/updateResearch?id="+id+"&restate="+restate;
        }
