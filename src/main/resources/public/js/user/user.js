layui.use(['table','layer'],function(){
    ...
    var tableIns = table.render({
        ...
        ,cols: [[
            ...
            ,{title:'操作',templet:'#userListBar', fixed: 'right', align:'center', minWidth:300}
        ]]
    });
    ...
    $(".search_btn").click(function () {
        ...
        tableIns.reload({
            ...
            where: { //设定异步数据接口的额外参数，任意设
                ...
                ,email: $("[name='usertruename']").val() // 邮箱
            }
            ,page: {
                curr: 1 // 重新从第 1 页开始
            }
    ...
    function updatePwd(uid){
        var title = "<h3>用户管理 - 修改密码</h3>";
        var url = ctx + "/user/toPasswordPage?uid="+uid;
        ...
        layer.confirm('您确定要删除选中的记录吗？',{icon:3, title:'用户管理'}, function (index) {
            ...
            for(var i = 0; i < userData.length; i++) {
                if(i < userData.length -1) {
                    ids = ids + userData[i].uid + "&ids="
                } else {
                    ids = ids + userData[i].uid;
                }
            }
            // console.log(ids);
    ...
    table.on('tool(users)', function (data) {
        ...
        if (data.event == "edit") { // 更新用户
            ...
            // 打开添加/修改用户的对话框
            openAddOrUpdateUserDialog(data.data.uid);

        } else if (data.event == "del") { // 删除用户
            // 删除单条用户记录
            deleteUser(data.data.uid);
        } else if(data.event == "updatePwd"){
            //修改用户密码
            updatePwd(data.data.uid);
        }
