layui.use(['form', 'layer' ,'upload','jquery','element'], function () {
     ...
     * 表单Submit监听
     */
    form.on('submit(addOrUpdateProject)', function (data) {
        var isFinished;
        if($("input[name='pcondition']").val()=="1"){
            isFinished=true;
        }else{
            isFinished=false;
        }
        ...
        // 请求的地址
        var url = ctx + "/project/addProject?isFinished="+isFinished; // 添加操作
        ...
        if ($("#pid").val()) {
            var pid = $("#pid").val();
            // 更新操作
            url = ctx + "/project/updateProject?isFinished="+isFinished+"&pid="+pid;
        }
