layui.use(['table','layer'],function(){
       ...
       var tableIns = table.render({
              ...
              ,cols: [[
                     ...
                     ,{field: 'rid', title: '编号',  sort: true, fixed: 'left'}
    ...
    table.on('tool(roles)',function (data) {
        ...
        if (data.event == "edit") { // 修改角色
            // 打开添加/更新角色的对话框
            openAddOrUpdateRoleDialog(data.data.rid);
        } else if (data.event == "del") {
            // 删除角色
            deleteRole(data.data.rid);
        }
    });
   ...
   function openAddOrUpdateRoleDialog(roleId) {
    ...
    function openAddGrantDialog(data) {
        ...
        var url = ctx + "/module/toAddGrantPage?roleId="+data[0].rid;
